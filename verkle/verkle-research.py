...
def update_verkle_tree(root_node, key, value):
    ...
    while True:
        ...
        # If the index is in the current node, then it means we are modifying
        # a node which is already in the trie
        if index in current_node:
            # If the node is a suffix tree, then we need to modify the stem extension
            # Its possible that we may need to add inner nodes, if stem to insert does
            # not match the stem of the suffix tree completely
            if current_node[index]["node_type"] == VERKLE_TRIE_NODE_TYPE_SUFFIX_TREE:
                ...
                if current_node[index]["stem"] == stem:
                    ...
                    # Update the extension commitment
...
def make_ipa_multiproof(Cs, fs, zs, ys, display_times=True):
    ...
    y, ipa_proof = ipa_utils.evaluate_and_compute_ipa_proof(
