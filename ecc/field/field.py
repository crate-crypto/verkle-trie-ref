...
# A generic implementation of a field element
...
class Field():
    ...
    def to_bytes(self, byte_length):
        return self.value.to_bytes(byte_length, byteorder='little')

    def from_bytes(bytes_little_endian, modulus):
    ...
    def from_bytes_reduce(bytes_little_endian, modulus):
        value = int.from_bytes(
            bytes_little_endian, byteorder='little')
        return Field(value, modulus)

    def lexographically_largest(x, q_min_one_div_2):
    ...
    def _check_all_integers_same_modulus(self, a, b):
